...
install_shadowbox() {
  ...
  if [[ ! "$SB_PUBLIC_IP" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
    ...
    log_step "Invalid IP lookup result"
    exit 1
  fi
  ...
  # this file.
  log_step "Initializing ACCESS_CONFIG"
